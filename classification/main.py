...
def conceptFromOpendmap(textId, drugPairs):
	tmp =0
	try:
		opendmapFile = open('./opendmapOutput/'+textId)
...
def main():
	...
	f = open('dataset.txt', 'wr')
	for xmlFile in listOfFiles:
		
		tree = ET.parse('./Train/drugBankMedLine/'+ xmlFile)
		root = tree.getroot()	
		...
		for child in root:
			...
			for p in child.iter('pair'):
				...
				if p.attrib['ddi'] == 'true':# and p.attrib['type'] == 'effect':
					y.append(1)
					'''
	    ...
	    """ 	
	f.close()
	pickle.dump(X,open("train_feature_vector.p","wb"))
	pickle.dump(y,open("train_values.p","wb"))
	print pair_count

	...
	for xmlFile in listOfFiles:
		...
		for child in root:
			...
			for p in child.iter('pair'):
				...
				pairs_dictionary[pair_count]={}
				pairs_dictionary[pair_count]=(pairs_id,p1,p2,0)
				if p.attrib['ddi'] == 'true': #:and p.attrib['type'] == 'int':
					#print "1"
					#print "%s:%s" %(p.attrib["id"],p.attrib["ddi"])
					y.append(1)
					'''
				...
				feature_for_a_pair=[]
				#+ ddiType(text, charOffset[0], charOffset[1]
				feature_for_a_pair=distance2Drugs(text, charOffset[0], charOffset[1]) + distanceBigrams(text, charOffset[0], charOffset[1])
				#print "______________________"
				#pprint.pprint(feature_for_a_pair)
				X.append( feature_for_a_pair)
		"""
		tmp = 0
		for i in X:
			#f.write(str(y[tmp]))
		...
		"""
	f.close()
	pickle.dump(X,open("test_feature_vector.p","wb"))
	pickle.dump(y,open("test_values.p","wb"))
	pickle.dump(pairs_dictionary,open("pairs_dictionary.p","wb"))
